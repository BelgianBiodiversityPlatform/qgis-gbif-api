...
triggered by pushing a version tag; GitHub Actions then builds the zip, creates
a GitHub release, and publishes the plugin to the official QGIS plugin
repository at [plugins.qgis.org](https://plugins.qgis.org).
...
2. **Tag the release with the bare version number** (no `v` prefix) and push the
   tag:
...
3. **Watch the workflow.** Pushing the tag triggers
...
Add `--github-token "$GITHUB_TOKEN"` if you also want it to create the GitHub
release. Omit the OSGeo flags to only build/attach the zip without touching
plugins.qgis.org.
